...
int main()
{
    ...
    if(mFile.is_open()){ // Verifies if file is successfully opened
        ...
        while (mFile >> number[i] && i <= count) { // Loop reads each integer and stores them on the array 
            if(i > 3 && (number[i-1] + number[i-2] + number[i-3]) < (number[i] + number[i-1] + number[i-2])){ 
                // Adds to the counter of increases when the previous sum of 3 numbers is lower than the current one, starts at i = 1
                increased++;
